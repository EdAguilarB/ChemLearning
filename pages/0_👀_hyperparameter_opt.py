...
with st.expander('Step 2: Select Hyperparameter Search Space', expanded=False):
    ...
    emb_dim_range = st.multiselect("Select range of embedding dimensions to be explored", options=[32, 64, 128, 256])

    conv_operator = st.selectbox("Select convolutional operators to be explored", options=['GCNConv', 'GATConv', 'SAGEConv', 'ChebConv'], index=None)

    n_convs_range = st.multiselect("Select range of convolutional layers to be explored", options=list(range(2, 6)))

    readout_layers_range = st.multiselect("Select range of readout layers to be explored", options=list(range(1, 6)))

    pooling_range = st.multiselect("Select range of pooling operations to be explored", options=['Mean', 'Max', 'Sum'])


    st.subheader("Select Model Training Hyperparameter Search Space")

    epochs_range = st.multiselect("Select range of epochs to be explored", options=list(range(50, 301, 50)))

    if len(epochs_range) > 3:
        st.warning("Selecting a large range of epochs can significantly increase computational time.")
    
    learning_rate_range = st.multiselect("Select range of learning rates to be explored", options=[0.001, 0.005, 0.01, 0.05, 0.1])
    ...
    early_stopping_range = st.multiselect("Select range of early stopping epochs to be explored", options=[1, 2, 3, 4, 5])
    

    batch_size_range = st.multiselect("Select range of batch sizes to be explored", options=[16, 32, 64])
...
if st.button("Run Hyperparameter Optimization"):
    ...
    configs = {
        ...
        'early_stopping_patience': tune.choice(early_stopping_range),
    ...
    if split_type == "Train-Validation-Test Split":
        configs['val_size'] = val_set_ratio
        configs['test_size'] = test_set_ratio
    else:
        configs['folds'] = folds
    ...
    st.download_button(
        ...
        data=json_file,
        file_name=f"best_hyperparameters_{opt.experiment_name}.json",
